...
function pms_payment_error_message_retry( $is_register, $payment_id = 0 ) {
    ...
    if ( $is_register == '1' ) {
        ...
        if ( !is_user_logged_in() ) {
            ...
            $message = sprintf( __( 'You were logged in, so please %sgo back%s to retry the payment.', 'paid-member-subscriptions' ), '<a href="javascript:history.back(1);">', '</a>' );
    ...
    //user is logged in
        ...
        if ( $payment_id != 0 && pms_get_page( 'account' ) != false ) {
            ...
            $message = sprintf( __( 'Please %sclick here%s to try again.', 'paid-member-subscriptions' ), '<a href="'. pms_get_retry_payment_url( $payment->subscription_id ) .'">', '</a>' );
